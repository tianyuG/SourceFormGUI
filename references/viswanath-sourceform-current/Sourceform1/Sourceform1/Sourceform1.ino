...
//-------------- Print Parameters-----------------------------------------------------------------------------------------
float burnin = 5000; // burnin time (milli s) for first layer
float exptime = 550; // exposure time in millisec for 0.100 mm layer in seconds
float heightperprojection = .025; //in milimeters
float projectionspersecond = 1/exptime; //in Hz
int microsteppingfactor = 1;

//-------------- Machine Parameters --------------------------------------------------------------------------------------

const float rapidSpeedmmpers = 15; //in milimeters per second
const float hometostartdist = 293.00; //nominal distance from home position to bottom in milimeters 287.38
...
const float printSpeedmmpers = heightperprojection/exptime; //in milimeters per second
...
}//remember to add the communication part here
...
void loop() {
  ...
  while (Serial.available()) {
    ...
    if (input.charAt(0) == 'b')
    {
      burnin = input.substring(1).toInt();
      //setting the burnin time from program
      Serial.write("done");
    }
    if (input.charAt(0) == 'o')
    {
      exptime = input.substring(1).toInt();
      //Setting the exposure time from program
      Serial.write("done");
    }
...
//      myStepper.step(hometostartsteps - currentLoc);
    ...
    if (input.charAt(0) == 'p')
    {
      ...
      for(i= 1; i<=((numImages*4)+3000); i++)  //Loop the forward stepping enough times for motion to be visible (PS Remember to change fudge factor to 100)
      {
        if (i==1)
        {
          delay(burnin);//burn in platform}
        }
        else
        {
          digitalWrite(stp,HIGH); //Trigger one step forward
          delay(exptime/4); //works with 5.5v in
          digitalWrite(stp,LOW); //Pull step pin low so it can be triggered again
        }
      }
      while (digitalRead(limitSwitchPin) == HIGH)

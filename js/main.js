...
// { name, ts, status, localpath, remotepath }
// name - <String> name of the job (can contain space)
// ts: <Date> timestamp. 
//     - Use global.jobs[index].ts.getTime() for numeric value
...
//     - modelling: 2
//     - printing: 3
// localpath: <String> absolute path of the job folder on local computer
// remotepath: <String> absolute path of the job folder on remote computer
global.jobs = []
/*
...
// TODO: Print an existing model (clicking on the carousel)
ipcMain.on('request-existing-model', (event, data) => {
	let modelName = data["n"]
	let modelPath = data["p"]
})
...
ipcMain.on('worker-download-search-r', (event, data) => {
	...
	if (global.jobs.some((e) => { return e["status" > 0] })) {
		global.jobs.push({name: name_o, timestamp: ts, status: -255, localpath: path.resolve(global.imagePath, "./" + name_p + "_" + ts.getTime()), remotepath: path.resolve(global.remotePath, "./" + name_p + "_" + ts.getTime())})
	} else {
		global.jobs.push({name: name_o, timestamp: ts, status: 1, localpath: path.resolve(global.imagePath, "./" + name_p + "_" + ts.getTime()), remotepath: path.resolve(global.remotePath, "./" + name_p + "_" + ts.getTime())})
	}
